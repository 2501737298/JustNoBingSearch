...
class BingSearchsApp:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("JustNoBingSearch v1.4")
        ...
        self.use_edge_checkbox = tk.Checkbutton(
            self.root, text="是否使用Edge浏览器", variable=self.auto_open_edge, command=self.on_edge_checkbox_change)
        self.use_edge_checkbox.pack()

        self.auto_open_edge_disabled = tk.BooleanVar(value=True)
        self.auto_open_edge_checkbox = tk.Checkbutton(
            self.root, text="若Edge窗口不存在，自动打开Edge(目前该选框无效)", variable=self.auto_open_edge_disabled, state=tk.DISABLED)
        self.auto_open_edge_checkbox.pack()

        self.consent_window = ConsentWindow(
            tk.Toplevel(), self.on_consent_given)

    def on_edge_checkbox_change(self):
        if self.auto_open_edge.get():
            self.auto_open_edge_checkbox.config(state=tk.NORMAL)
        else:
            self.auto_open_edge_checkbox.config(state=tk.DISABLED)
            self.auto_open_edge_disabled.set(True)
    ...
    def get_target_window(self):
        ...
        if self.auto_open_edge.get():
            ...
            self.prompt_label.config(text="请在四秒内点击目标窗口")
            self.root.update()
            time.sleep(4)
